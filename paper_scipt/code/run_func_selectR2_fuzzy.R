run.func_V = function(l){
  ...
  R2_cutoff = r2cut.tab$r[r2cut.tab$data.idx == data.idx& r2cut.tab$orig_V == V]
  ...
  invisible(capture.output({  out = tryCatch({
    ...
    if(length(init.ls) != 0){
      ...
      res = EM_multiView(V, K, pV, pVK, mu_GK_V, sigma_GV, std.data, 
                         GCthres_num = 5,R2_cutoff = R2_cutoff,
      ...
      if(res$V < 2){
        ...
        res.tb = c(mu0 = mu0, R2_cutoff=R2_cutoff, orig_V = V, initial.kmin=initial.kmin, initial.seednum=initial.seednum,
                   max_pairARI=NA,avg_pairARI=NA,V=res$V,NGclust=sum(pred_Glb != 0),
                   avgLLik_selected_soft=res$avgLLik_selected_soft,avgLLik_selected_hardView = res$avgLLik_selected_hardView,
                   ...
                   avgLLik_selected_soft_sepV=res$avgLLik_selected_soft_sepV,avgLLik_selected_hardView_sepV = res$avgLLik_selected_hardView_sepV,
        ...
        metric.tb = t(sapply(1:res$V, function(v){
          ...
          true.gene.lb1 = true.gene.lb2 = rep(0, nrow(std.data))
          true.gene.lb1[gene.lb01[[orig.idx]]] = 1
          true.gene.lb1 = factor(true.gene.lb1)
          
          true.gene.lb2[gene.lb02[[orig.idx]]] = 1
          true.gene.lb2 = factor(true.gene.lb2)
          
          mat1 = confusionMatrix(binary_lb,true.gene.lb1,positive = "1")
          G_Sensitivity1 = mat1$byClass["Sensitivity"]
          G_Specificity1 = mat1$byClass["Specificity"]
          
          mat2 = confusionMatrix(binary_lb,true.gene.lb2,positive = "1")
          G_Sensitivity2 = mat2$byClass["Sensitivity"]
          G_Specificity2 = mat2$byClass["Specificity"]
          return(c(v = v, orig.idx = orig.idx, Cl_ARI = max(ari_res), NG = sum(pred_Glb == v),
                   G_Sensitivity1, G_Specificity1, G_Sensitivity2, G_Specificity2))
        }))
        ShannonIdx = apply(res$postGV,1,function(xx) -sum(xx*log(xx)))
      ...
      aout = list(res = res,res.tb = res.tb,metric.tb = metric.tb,ShannonIdx=ShannonIdx)
    }
    aout
  },error = function(e) {
    return(list(e = e, l=l))
