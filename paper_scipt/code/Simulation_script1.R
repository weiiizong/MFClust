...
for (mu0 in c(0.8,1,1.2)) {
  ...
  save(full.res,file = paste0("~/output/Simulation_NG1000N200_homoSignal_V12345_mu",mu0,"_r20.RData"))
  
  res.tb.ls = lapply(full.res, "[[","res.tb")
  res.tb = data.frame(do.call(rbind,res.tb.ls))
  #res.tb[is.na(res.tb[,"avgR2_selected_soft_sepV"]),"avgR2_selected_soft_sepV"] = 0
  library(dplyr)
  df01 = res.tb %>% 
  ...
  save(r2cut.tab,
       file = paste0("~/output/Simulation_NG1000N200_homoSignal_V12345_mu",mu0,"_r2cut.RData"))
}
...
for (mu0 in c(0.8,1,1.2)) {
  ...
  full.res = mclapply(1:nrow(scenario.idx),run.func_V,mc.cores = 50)
  save(full.res,file = paste0("~/output/Simulation_NG1000N200_homoSignal_V12345_mu",mu0,"_selectR2.RData"))
}