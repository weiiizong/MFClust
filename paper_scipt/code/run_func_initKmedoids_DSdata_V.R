run.func_V = function(l){
  ...
  G.K = G.K.list[scenario.idx[l,3]]
  
  V = V.list[[scenario.idx[l,4]]]
  ...
  out = tryCatch({
    ...
    if(length(init.ls) != 0){
      ...
      res = EM_multiView(V, K, pV, pVK, mu_GK_V, sigma_GV, std.data, 
                         ...
                         quite=F, updateK = T, updateK_thin = 1, maxr = 200)
      if(res$V < 2){
        res.tb = c(orig_V = V, G.K = G.K, R2_cutoff=R2_cutoff, 
                   initial.kmin=initial.kmin, initial.seednum=initial.seednum,
                   ...
                   initial.idx=initial.idx,R2.idx=R2.idx,l=l)
        metric.tb = NA
      }else{
        ...
        metric.tb = sapply(cl.ls, function(cl){
          ...
          Race = fisher.test(clinical$Race.Label,cl)$p.value
          ...
          AUD = fisher.test(clinical$AUD,cl)$p.value
          ...
          TOD = summary(lm(clinical$CorrectedTOD ~ cl))$coefficients[2,4]
          
          return(c(Sex = Sex, Race = Race,
                   Disease = Disease, AUD = AUD, 
                   TOD = TOD,  Age = Age, PMI = PMI, pH = pH, RIN = RIN))
        })
